...
— and let it pick one per turn. juju gives it a Python kernel instead. The model writes code,
...
- **Project instructions** — drop a `JUJU.md` in your working directory and its contents
...
Then either activate the venv, or put `juju` on your PATH once:

```bash
ln -sf "$PWD/.venv/bin/juju" /opt/homebrew/bin/juju
```
